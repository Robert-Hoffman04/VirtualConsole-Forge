...
/**
 * Evaluate every option for the given controller and current values.
 * Returns, in registry order:
 *   { option, value, shown, enabled }
 *  - shown:   false when its visible_when condition isn't met (row hidden)
 *  - enabled: false when the option doesn't apply to this controller (row
 *             shown but disabled, with an explanation)
 *  - value:   the effective value (the default when not applicable)
 */
export function evaluateOptions(core, device, values) {
  const effective = {};
  const out = [];
  for (const option of core?.options ?? []) {
    ...
    const deviceOk = !option.devices?.length || option.devices.includes(device);
...
export function effectiveValues(core, device, values) {
  return Object.fromEntries(evaluateOptions(core, device, values).map((s) => [s.option.id, s.value]));
}
...
export function summarizeOptions(core, device, values) {
  if (!core?.options?.length) return "None available";
  const changed = evaluateOptions(core, device, values)
